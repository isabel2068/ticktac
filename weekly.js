...
const CURRENT_USER_ID = String(window.CURRENT_USER_ID);
...
window.loadWeeklyTasks = async function () {
    ...
    try {
        ...
        const ongoingCont = document.getElementById("ongoingContainer");
        const pendingCont = document.getElementById("pendingContainer");
        const completedCont = document.getElementById("completedContainer");
        ...
        if (!tasks || !tasks.length) {
            ...
            toggleWrapper("ongoingWrapper", ongoingCont);
            toggleWrapper("pendingWrapper", pendingCont);
            toggleWrapper("completedWrapper", completedCont);
        ...
        tasks.forEach((task) => {
            ...
            const canEditChecklist =
                assignedUsers.includes(CURRENT_USER_ID);
            ...
            let doneCount =
                checklist.filter(c => c.done == 1).length;
            ...
            const collapseId = `weeklyTask${task.id}`;
...
// UPDATE COUNTER
function updateChecklistCounter(taskId) {

    const checkbox =
        document.querySelector(
            `.checklist-item[data-task-id="${taskId}"]`
        );

    const card =
        checkbox
            ? checkbox.closest(".list-group-item")
            : null;

    if (!card) return;

    const checkboxes =
        card.querySelectorAll(".checklist-item");
    ...
    const counter =
        card.querySelector(".task-counter");
    ...
    if (counter) {
        ...
        if (total === 0) {
            ...
            counter.innerHTML =
                `<span class="fw-bold" style="font-size:14px;">${done}</span><span style="font-size:10px;"> / ${total}</span>`;
        }
    }
}